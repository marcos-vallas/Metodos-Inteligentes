...
@export var distance_to_chase: float = 200.0  # Rango para empezar a perseguir (Seek)
@export var distance_to_arrive: float = 50.0  # Rango para empezar a frenar (Arrive)

# Asegurate de tener un RayCast2D llamado RayCastCentral como hijo del Chaser
...
func movement_loop() -> void:
	if player_node:
		...
		# Transiciones de estado (Punto 6 de la consigna)
		if dist < distance_to_arrive:
			current_behavior = State.ARRIVE
		elif dist < distance_to_chase:
			current_behavior = State.SEEK
		else:
			current_behavior = State.WANDER

	# Ejecución del comportamiento según el estado activo
	match current_behavior:
		State.WANDER:
			self.velocity = movement_comp.wander(global_position, movement_comp.move_direction, raycast)
		State.SEEK:
			self.velocity = movement_comp.seek(global_position, player_node.global_position)
		State.ARRIVE:
			self.velocity = movement_comp.arrive(global_position, player_node.global_position)
